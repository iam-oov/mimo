    ...
    def create() -> LanguageModelProvider:
        ...
        try:
            ...
            try:
                ...
                raise ValueError("No AI provider available. Set DEEPSEEK_API_KEY or GEMINI_API_KEY")
...
- Tu respuesta debe tener EXACTAMENTE entre 450-500 caracteres
...
- Usa tu personalidad {self.profile.personality_traits.name} en tus argumentos"""
    ...
    def generate_analysis(
        ...
        messages = [
            ...
            {"role": "user", "content": f"""
Estás en una mesa de análisis fiscal con {others_names}.
...
Proporciona tu análisis profesional con una recomendación específica para mejorar la situación fiscal.
IMPORTANTE: Tu respuesta debe tener entre 450-500 caracteres."""}
        ]
    ...
    def vote(
        ...
        messages = [
            {"role": "system", "content": self._build_system_prompt()},
            {"role": "user", "content": f"""
Basándote en la discusión, vota por la mejor estrategia.
...
Responde SOLO con el número (1, 2 o 3) de la estrategia que consideras mejor desde tu perspectiva profesional como {self.profile.profession_traits.title}.
...
class ModeratorAgent:
    ...
    def _build_system_prompt(self) -> str:
        ...
        return """Eres un Moderador Fiscal neutral y eficiente.
...
RESTRICCIONES:
- Tus intervenciones deben tener entre 450-500 caracteres
...
IMPORTANTE: Tu introducción debe tener entre 450-500 caracteres."""}
        ]
...
Destaca las propuestas principales de cada experto sin favorecer ninguna.
IMPORTANTE: Tu resumen debe tener entre 450-500 caracteres."""}
        ]
...
IMPORTANTE: Tu anuncio debe tener entre 450-500 caracteres."""}
        ]
...
IMPORTANTE: Tu conclusión debe tener entre 450-500 caracteres."""}
        ]
...
class AgentFactory:
    ...
    def create_random_experts(count: int = 3) -> List[FiscalExpertAgent]:
        ...
        expert_names = [
            "Dr. Martínez", "Lic. García", "Mtra. López",
            "Ing. Rodríguez", "C.P. Hernández", "Dr. Ramírez"
        ]
...
class MultiAgentConversationOrchestrator:
    ...
    def _get_expert_names(self) -> List[str]:
        ...
        return [f"{e.profile.name} ({e.profile.profession_traits.title})" for e in self.experts]
    ...
    def _voting_phase(self) -> Dict[str, Any]:
        ...
        for expert in self.experts:
            vote_response = self._collect_stream(
                expert.vote(self.conversation_history, expert_strategies)
            )
            
            try:
                vote_num = int(''.join(filter(str.isdigit, vote_response[:10])))
                if 1 <= vote_num <= len(self.experts):
                    voted_expert = self.experts[vote_num - 1].profile.name
                    votes.append(voted_expert)
                    vote_details.append({
                        "voter": expert.profile.name,
                        "voted_for": voted_expert
                    })
            except (ValueError, IndexError):
                votes.append(self.experts[0].profile.name)
                vote_details.append({
                    "voter": expert.profile.name,
                    "voted_for": self.experts[0].profile.name
                })
        
        vote_counts = Counter(votes)
        winner = vote_counts.most_common(1)[0][0] if vote_counts else self.experts[0].profile.name
        
        winning_expert = next((e for e in self.experts if e.profile.name == winner), self.experts[0])
...
class MultiAgentAnalysisService:
    ...
    def run_analysis_stream(
        ...
        orchestrator._introduction_phase()
        yield {
            "type": "intervention",
            "agent": moderator.name,
            "content": orchestrator.conversation_history.split(f"{moderator.name}: ")[-1].split("\n")[0]
        }
        ...
        for round_num in range(1, 4):
            ...
            full_summary = []
            for chunk in moderator.summarize_round(round_num, orchestrator.conversation_history):
